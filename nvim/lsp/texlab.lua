...
local function buf_cancel_build(client, bufnr)
	...
	local tex_client = vim.lsp.get_clients({ bufnr = bufnr, name = 'texlab' })[1]
	tex_client:exec_cmd { title = 'texlab.cancelBuild', command = 'texlab.cancelBuild' }
...
local function buf_change_env(client, bufnr)
	...
	local tex_client = vim.lsp.get_clients({ bufnr = bufnr, name = 'texlab' })[1]
	tex_client:exec_cmd {
		title = 'texlab.changeEnvironment',
		command = 'texlab.changeEnvironment',
		arguments = {
			{
				textDocument = { uri = vim.uri_from_bufnr(bufnr) },
